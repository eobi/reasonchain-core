...
**Manual penetration testing.** The OWASP Testing Guide and PTES
specify a methodology that human experts follow; their effectiveness
is gated by the workforce shortage [2]. Studies of human testers
[1] show that even experienced operators miss multi-step attack
chains because they cannot hold all tool outputs in working memory.

**Fixed automation.** Frameworks such as OWASP ZAP automation,
nuclei chains, and `bash`-orchestrated scripts can run dozens of
tools in sequence but cannot replan when a finding changes what the
operator should do next. Their structure is open-loop.

**LLM-assisted assessment.** PentestGPT [3] is the closest published
work: an LLM that proposes the next command, which a human then
runs. The paper explicitly describes the human as "the executor."
The system never observes the actual output or replans against it
autonomously.

**Cyber Reasoning Systems (CRS).** DARPA's Cyber Grand Challenge
(CGC) and the more recent AI Cyber Challenge (AIxCC) [6] funded
end-to-end autonomous CRSes that find, exploit, and patch
vulnerabilities. CRSes operate on compiled binaries and focus on
memory-corruption classes (heap-spray, ROP); ReasonChain operates
on running web applications and focuses on detection. The two
architectures share the closed-loop principle but address
non-overlapping problem spaces.

**Multi-agent LLM frameworks.** AutoGPT, MetaGPT, BabyAGI, and
related projects demonstrate autonomous LLM tool use in general
domains. None has been evaluated on cybersecurity assessment with
the kind of ablation rigor we present here.

# 3 The ReasonChain Architecture

Figure 1 shows the end-to-end loop. Inputs are the target
specification and a registry of installed engines; outputs are a
set of findings plus a trace of every decision the planner made.
...
and feeds the parsed result back into the planner. We have not run
a head-to-head benchmark (the published PentestGPT artifact does
not provide a reproducible execution harness for our targets);
this is concrete future work.
