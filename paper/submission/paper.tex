...
p=1.536e-24, Cohen d=\textbf{Z}), including real CVE-class findings
(e.g., CVE-2024-38476, CVE-2024-38474, CVE-2023-25690; all CVSS 9.8)
discovered by the agent's NSE-script invocations. We also introduce a
deterministic decision-quality annotator that labels every planner
decision as correct, suboptimal, or incorrect, providing the first
quantitative failure-mode taxonomy for autonomous web-app assessment
agents. We release the reference implementation (MIT-licensed at
...
Cyber Grand Challenge (2016) and AI Cyber Challenge (2024) {[}6{]}
...
\item
  \textbf{An empirical evaluation} across 30 OWASP-class targets and 72
...
The 120-cell matrix completed in 5.4 hours of wall-clock time. Across
all 18 targets:
...
yields W = 465, p = 0.0000. The paired t-test, excluding the DVWA
outlier (which emits \textgreater2000 findings under full due to the
breadth of its built-in test surface), yields t = 33.79, p = 1.536e-24,
Cohen d = 6.27.
...
and fact-coupled (see §3.2), the no-fusion condition produces 75.8\%
...
The paired t-test yields t = 33.34, p = 0.0000; Wilcoxon p = 0.0000. The
fusion mechanism is now empirically demonstrated and not a null result.
...
\subsection{6.6 Sensitivity to the DVWA outlier (engine-pool
ablation)}\label{sensitivity-to-the-dvwa-outlier-engine-pool-ablation}

The reported Cohen's d on H1 is sensitive to two effects: the extreme
value at DVWA (\textgreater2000 findings under \texttt{full}) and the
contribution of \texttt{nikto}, the engine that enumerates DVWA's
teaching endpoints. To isolate these effects we re-run the matrix with
\texttt{nikto} removed from the engine pool. \textbf{Results to be
re-rendered with \texttt{data/results\_no\_nikto.csv} after Tier B.2
completes.}

The intended interpretation: under the no-nikto pool the absolute
finding count drops but the H1 \emph{direction} (full \textgreater{}
no-replan) must remain positive on every target for the architectural
claim to survive. If the Wilcoxon p stays \textless{} 0.01 on the
no-nikto matrix, the architecture's effect cannot be attributed solely
to nikto's endpoint enumeration on DVWA.
...
Across 30 OWASP-class targets and 72 matrix cells, the closed-loop
